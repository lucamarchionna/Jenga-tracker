...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    while (!quit ) {
      ...
      // Define camera's RF
      vpTranslationVector t_cam; t_cam << 0.0, 0.0, 0.0;
      vpQuaternionVector q_cam, q_unit; q_cam << 0, 0, 0, 1;
      eeTc.buildFrom(t_cam, q_cam);
      geometry_msgs::TransformStamped camera_RF = toMoveit(eeTc, "camera_color_optical_frame", "object");
