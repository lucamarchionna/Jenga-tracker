...
void visual_servoing::estimationCallback(const geometry_msgs::Pose& tracker_pose_P) 
{
  ...
  while (!run_completed && node_handle.ok()) {
    if (!block_axis) {
      ...
      // IK_pose = move_group.getCurrentPose("edo_link_6").pose;
      // bTe6 = visp_bridge::toVispHomogeneousMatrix(IK_pose); 
      // bTc = bTe6*extrinsic;
      // camTbase = bTc.inverse();
      ...
      if (take_cTo) {
        ...
        if (distance_run < threshold_pose && retract == false )  {
          ...
          if (distance_run > threshold_pose/2) {
            signPullX = -1.0*signPullX;
            velocityData.twist.linear.y = velocityData.twist.linear.y + signPullX*0.007;
        ...
        else if (distance_run < 0 && retract == true)  {
...
void visual_servoing::forceCallback(const std_msgs::BoolConstPtr& retract_call) 
{
  ...
  // Retracting since f_max > threshold
  if (retract) {
...
void visual_servoing::init_startLoop()
{
  ...
  // Create new tracker and initialize with known config
...
void visual_servoing::init_shortLoop()
{
  ...
  if (opt_use_depth){   
    ...
    tracker->loadConfigFile(opt_config);
...
{ 
  ...
  camTee=homogeneousTransformation("edo_gripper_link_ee", "camera_color_optical_frame"); 
  camTbase = homogeneousTransformation("camera_color_optical_frame", "edo_base_link");
  extrinsic = homogeneousTransformation("edo_link_ee", "camera_color_optical_frame");
  ...
  try {
    ...
    while (!quit && node_handle.ok() ) {
      ...
      if (proj_error > opt_proj_error_threshold) {
        {
          std::stringstream ss;
          ss << "Tracker needs to restart (projection error detected: " << proj_error << ")" << std::endl;
          vpDisplay::displayText(I_color, 0, 20, ss.str(), vpColor::black);
        }               
      ...
      if (!tracking_failed) {
        ...
        if (opt_display_model){
          if(opt_use_depth){
            ...
            tracker->display(I_color, cMo, cam_color, vpColor::green, 1);
            vpDisplay::displayFrame(I_color, cMo, cam_color, 0.05, vpColor::none, 3);
          }
        }
      }
