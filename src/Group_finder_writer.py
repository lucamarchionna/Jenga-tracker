...
class Blocks_group():
    ...
    def __init__(self,blocks_list,startIdx,img):
        ...
        test_up,test_down,test_right0,test_right1,test_left0,test_left1=self.start_block.compute_test_points()
            ...
            if block_test.idx!=self.startIdx:
                ...
                #Test right0
                    ...
                    if self.right0_block.block_type=='front_face':
                        _,_,test_right1,_,_,_=self.right0_block.compute_test_points()
                ...
                #Test left0
                    ...
                    if self.left0_block.block_type=='front_face':
                        _,_,_,_,test_left1,_=self.left0_block.compute_test_points()
    ...
    def cao_file_write(self,cao_name,center_cao_path,right_cao_path,left_cao_path):
        ...
        with open(cao_name,"w") as file_cao:
            ...
            if self.rightIdx[0]!=-1 and self.leftIdx[0]!=-1:
                lines_to_write.append('load("'+self.center_cao_path+'", '+t_to_string(self.target_move_target)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_right0)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                lines_to_write.append('load("'+self.left_cao_path+'", '+t_to_string(self.target_move_left0)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                if self.upIdx!=-1:   #keep same T not depending on target block on right or left side; front and rear faces depends on it though
                    lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_upcenter)+', '+tu_to_string(self.target_RotVec_updown)+')\n')
                if self.downIdx!=-1:
                    lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_downcenter)+', '+tu_to_string(self.target_RotVec_updown)+')\n')
            
            elif self.rightIdx[0]!=-1 and self.rightIdx[1]!=-1:
                lines_to_write.append('load("'+self.left_cao_path+'", '+t_to_string(self.target_move_target)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                lines_to_write.append('load("'+self.center_cao_path+'", '+t_to_string(self.target_move_right0)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_right1)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                if self.upIdx!=-1:   #keep same T not depending on target block on right or left side; front and rear faces depends on it though
                    lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_upleft)+', '+tu_to_string(self.target_RotVec_updown)+')\n')
                if self.downIdx!=-1:
                    lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_downleft)+', '+tu_to_string(self.target_RotVec_updown)+')\n')
        
            elif self.leftIdx[0]!=-1 and self.leftIdx[1]!=-1:
                lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_target)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                lines_to_write.append('load("'+self.center_cao_path+'", '+t_to_string(self.target_move_left0)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                lines_to_write.append('load("'+self.left_cao_path+'", '+t_to_string(self.target_move_left1)+', '+tu_to_string(self.target_RotVec_target)+')\n')
                if self.upIdx!=-1:   #keep same T not depending on target block on right or left side; front and rear faces depends on it though
                    lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_upright)+', '+tu_to_string(self.target_RotVec_updown)+')\n')
                if self.downIdx!=-1:
                    lines_to_write.append('load("'+self.right_cao_path+'", '+t_to_string(self.target_move_downright)+', '+tu_to_string(self.target_RotVec_updown)+')\n')    
        
        #     ## Missing blocks conditions
        #     elif rightIdx[0]!=-1 and rightIdx[1]==-1 and leftIdx[0]==-1:
        #         #discriminate between two equal condition possibilities
        #         #1) Search up&down/right if block is on right face
        #             #found yes
        #                 lines_to_write.append('load("../blocks_cao/jengaLeftO.cao", t=[0.0; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 lines_to_write.append('load("../blocks_cao/jengaCenterO.cao", t=[0.025; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 if upIdx!=-1:   
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0125; 0.0125; 0.015], tu=[0.0; 0.0; -90.0 deg])')
        #                 if downIdx!=-1:
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0125; 0.0125; -0.015], tu=[0.0; 0.0; -90.0 deg])')
        #             #found no
        #                 lines_to_write.append('load("../blocks_cao/jengaCenterO.cao", t=[0.0; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[0.025; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 if upIdx!=-1:   
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0375; 0.0125; 0.015], tu=[0.0; 0.0; -90.0 deg])')
        #                 if downIdx!=-1:
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0375; 0.0125; -0.015], tu=[0.0; 0.0; -90.0 deg])')
        #         #2) Search up&down/left if block is on left face
        #             #found yes
        #                 lines_to_write.append('load("../blocks_cao/jengaCenterO.cao", t=[0.0; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[0.025; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 if upIdx!=-1: 
        ...
        #     elif rightIdx[0]!=-1 and rightIdx[1]==-1 and leftIdx[0]==-1:
        #         #SAME AS PREVIOUS BUT THE OTHER BLOCK AS TARGET
        #         #discriminate between two equal condition possibilities
        #         #1) Search up&down/right if block is on right face
        #             #found yes
        #                 lines_to_write.append('load("../blocks_cao/jengaCenterO.cao", t=[0.0; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 lines_to_write.append('load("../blocks_cao/jengaLeftO.cao", t=[-0.025; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 if upIdx!=-1:   
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0375; 0.0125; 0.015], tu=[0.0; 0.0; -90.0 deg])')
        #                 if downIdx!=-1:
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0375; 0.0125; -0.015], tu=[0.0; 0.0; -90.0 deg])')
        #             #found no
        #                 lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[0.0; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 lines_to_write.append('load("../blocks_cao/jengaCenterO.cao", t=[-0.025; 0.0; 0.0], tu=[0.0; 0.0; 0.0 deg])')
        #                 if upIdx!=-1:   
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0625; 0.0125; 0.015], tu=[0.0; 0.0; -90.0 deg])')
        #                 if downIdx!=-1:
        #                     lines_to_write.append('load("../blocks_cao/jengaRightO.cao", t=[-0.0625; 0.0125; -0.015], tu=[0.0; 0.0; -90.0 deg])')
        #         #2) Search up&down/left if block is on left face
        #             #found yes
        ...
        #     # elif rightIdx[0]!=-1 and lefidx[0]==-1 and leftIdx[0]!=-1:
        #     #     # missing central, other configuration
